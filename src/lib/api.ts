...
const API_BASE_URL = 'https://studyvalut.onrender.com';
...
export async function suggestOrganization(params: {
  ...
  try {
    const res = await fetch(
      `${API_BASE_URL}/api/gemini/suggest-organization`,
      {
        method: 'POST',
        headers: {
    ...
    console.error('AI organization error:', data.error);
    return null;
  } catch (err) {
    console.error(
      'Failed to get AI organization suggestions:',
      err
    );
    return null;
  }
...
export async function summarizePdf(params: {
  ...
  try {
    ...
    const res = await fetch(
      `${API_BASE_URL}/api/gemini/summarize-pdf`,
      {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
    ...
    if (!contentType.includes('application/json')) {
      ...
      console.error(
        'Non-JSON response from summarize-pdf:',
        text.slice(0, 300)
      );

      throw new Error(
        `AI server returned HTTP ${res.status}.`
      );
    }
...
export async function askPdfQuestion(params: {
  ...
  try {
    ...
    const res = await fetch(
      `${API_BASE_URL}/api/gemini/qa-pdf`,
      {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
    ...
    console.error(
      'Failed to ask question about PDF:',
      err
    );
